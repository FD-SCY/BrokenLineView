...
+ (PopoverView *)showPopoverAtPoint:(CGPoint)point inView:(UIView *)view withContentView:(UIView *)cView shadow:(UIColor *)shadow delegate:(id<PopoverViewDelegate>)delegate {
    PopoverView *popoverView = [[PopoverView alloc] initWithFrame:CGRectZero];
    [popoverView showAtPoint:point inView:view withContentView:cView];
    popoverView.delegate = delegate;
    popoverView.shadowColor = shadow;
    [popoverView RELEASE];
    return popoverView;
}
...
-(void)setupLayout:(CGPoint)point inView:(UIView*)view
{
    ...
    [tap RELEASE];
    // add by scy
    UISwipeGestureRecognizer *swip = [[UISwipeGestureRecognizer alloc]initWithTarget:self action:@selector(tapped:)];
